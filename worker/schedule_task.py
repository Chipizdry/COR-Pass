...
async def send_dvcc_max_charge_current_command(modbus_client: AsyncModbusTcpClient, charge_battery_value: int):
    """Отправка команды для установки DVCC max charge current."""
    if modbus_client is None: 
        return {"status": "error", "message": "Modbus client not available"}
