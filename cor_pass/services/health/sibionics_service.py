...
class SibionicsAPIClient:
    ...
    async def _refresh_token(self):
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                logger.info(f"✅ SIBIONICS merchant token obtained")
                logger.info(f"   Token: {self._access_token[:20]}... (expires: {self._token_expires})")
                logger.debug(f"📥 Full token response: {data}")
    ...
    async def _make_request(
        ...
        Args:
            ...
            json_data: JSON body
        """
        # Всегда используем merchant token
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                logger.info(f"📥 SIBIONICS API Response [{method} {endpoint}]: code={data.get('code')}, success={data.get('success')}")
    ...
    async def get_device_list(
        ...
        result = await self._make_request("GET", "/open/device/list", params=params)
        
        # Логируем количество найденных устройств
        records = result.get("records", [])
        logger.info(f"   Found {len(records)} devices (total: {result.get('total', 0)}, page: {result.get('currentPage')}/{result.get('totalPage')})")
        if records:
            logger.debug(f"   Device IDs: {[d.get('deviceId') for d in records]}")
    ...
    async def get_device_glucose_data(
        ...
        Args:
            biz_id: SIBIONICS Authorization resource ID
            device_id: ID устройства (SIBIONICS deviceId)
        ...
        logger.info(f"📊 Fetching glucose data for bizId: {biz_id}, device: {device_id}, index: {index}")
        result = await self._make_request("GET", "/open/device/glucose", params=params)
        
        # Логируем количество записей глюкозы
        records = result.get("records", [])
        logger.info(f"   Retrieved {len(records)} glucose records (total: {result.get('total', 0)}, page: {result.get('currentPage')}/{result.get('totalPage')})")
        if records:
            # Логируем первую и последнюю запись для контроля диапазона
            first = records[0]
            last = records[-1]
            logger.debug(f"   Index range: {first.get('i')} to {last.get('i')}, values: {first.get('v')} to {last.get('v')} mmol/L")
    ...
    async def revoke_authorization(self, biz_id: str) -> str:
        ...
        logger.info(f"   Revoke result: {result}")
        return result
    ...
    async def get_user_actions(
        ...
        """
        Получает действия пользователя (еда, спорт, лекарства, инсулин, сон, finger blood, самочувствие)
        
        Args:
            biz_id: SIBIONICS Authorization resource ID
            action_type: Тип действия (1: еда, 2: спорт, 3: лекарства, 4: инсулин, 5: сон, 6: fingerBlood, 7: самочувствие)
            begin_time: Время начала действия (milliseconds timestamp)
            page_num: Номер страницы
            page_size: Размер страницы (макс 1000)
        ...
        if action_type is not None:
            params["type"] = str(action_type)
        
        if begin_time is not None:
            params["beginTime"] = begin_time
        
        logger.info(f"🎯 Fetching user actions for bizId: {biz_id}, type: {action_type or 'all'}")
        result = await self._make_request("GET", "/open/action/list", params=params)
        
        # Логируем количество действий
        records = result.get("records", [])
        logger.info(f"   Found {len(records)} actions (total: {result.get('total', 0)}, page: {result.get('currentPage')}/{result.get('totalPage')})")
        if records:
            action_types = {}
            for r in records:
                t = r.get('type')
                action_types[t] = action_types.get(t, 0) + 1
            logger.debug(f"   Action types distribution: {action_types}")
