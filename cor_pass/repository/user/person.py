...
from cor_pass.repository.shared.password_generator import generate_password
...
from cor_pass.services.shared import roles as role_check
from loguru import logger
from cor_pass.services.user.cipher import (
    generate_aes_key,
    encrypt_user_key,
...
async def get_user_by_uuid(uuid: str, db: AsyncSession) -> User | None:
    ...
    """
    stmt = select(User).where(User.id == uuid).options(selectinload(User.profile))
    result = await db.execute(stmt)
    user = result.scalar_one_or_none()
    return user
