...
async def create_new_corid(user: User, db: AsyncSession):
    # Handle None values: use empty string instead of "None"
    birth_str = str(user.birth) if user.birth is not None else ""
    user_sex_str = str(user.user_sex) if user.user_sex is not None else ""
    birth_year_gender = f"{birth_str}{user_sex_str}"
...
async def create_only_corid(birth: int, user_sex: str, db: AsyncSession):
    # Handle None values: use empty string instead of "None"
    birth_str = str(birth) if birth is not None else ""
    user_sex_str = str(user_sex) if user_sex is not None else ""
    birth_year_gender = f"{birth_str}{user_sex_str}"
    jan_first_2024 = datetime(2024, 1, 1).date()
    today = datetime.now().date()
    n_days_since_first_jan_2024 = (today - jan_first_2024).days
