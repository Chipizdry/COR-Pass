...
from cor_pass.services.shared.access import lawyer_access, admin_access
...
from cor_pass.repository.doctor import lawyer as repository_lawyer
...
async def get_all_doctors(
    ...
    **Получение списка всех врачей**\n
    ...
    def calculate_age(doctor) -> Optional[int]:
        """Вычисляет возраст врача из Profile.birth_date или User.birth (год рождения)"""
        today = datetime.today()
        
        # Попытка получить точную дату рождения из профиля
        if doctor.user and doctor.user.profile and doctor.user.profile.birth_date:
            birth_date = doctor.user.profile.birth_date
            age = today.year - birth_date.year - ((today.month, today.day) < (birth_date.month, birth_date.day))
            return age
        
        # Fallback: использовать год рождения из User.birth
        if doctor.user and doctor.user.birth:
            return today.year - doctor.user.birth
...
@router.get("/doctors/{doctor_id}/reserv-scan", dependencies=[Depends(lawyer_access)])
async def get_doctor_reserv_scan(doctor_id: str, db: AsyncSession = Depends(get_db)):
    """Получает скан резерва врача (изображение или PDF) из базы данных."""
    doctor = await repository_lawyer.get_doctor(doctor_id=doctor_id, db=db)
    if not doctor or not doctor.reserv_scan_data or not doctor.reserv_scan_file_type:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Doctor or reserv scan not found"
        )

    async def reserv_scan_stream():
        yield doctor.reserv_scan_data

    return StreamingResponse(reserv_scan_stream(), media_type=doctor.reserv_scan_file_type)
...
async def get_doctor_with_relations(
    ...
    **Получение информации о враче со всеми связями**\n
    ...
    doctor_response = DoctorWithRelationsResponse(
        ...
        passport_code=doctor.passport_code,
        taxpayer_identification_number=doctor.taxpayer_identification_number,
        reserv_scan=f"/doctors/{doctor_id}/reserv-scan" if doctor.reserv_scan_data else None,
        date_of_next_review=doctor.date_of_next_review,
        scientific_degree=doctor.scientific_degree,
        date_of_last_attestation=doctor.date_of_last_attestation,
        status=doctor.status,
        diplomas=[
