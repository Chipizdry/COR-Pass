...
from cor_pass.repository.user import otp_auth as repository_otp_auth
...
from cor_pass.services.shared import cor_otp
...
async def read_otp_records(
    ...
    **Get a list of otp_records. / Получение всех otp записей пользователя** \n
    ...
    :return: A list of OTPRecordResponse objects representing the records with current OTP.
    ...
    for record in otp_records:
        otp_password, remaining_time = await cor_otp.generate_and_verify_otp(
            record.private_key, current_user
        )
...
async def read_otp_record(
    ...
    **Get a specific otp_record by ID. / Получение данных одной конкретной otp записи пользователя** \n
    ...
    :param db: The database session. Dependency on get_db.
    :type db: AsyncSession, optional
    :return: The OTPRecordResponse object representing the record with current OTP.
    :rtype: OTPRecordResponse
    :raises HTTPException 404: If the otp record with the specified ID does not exist.
    """
    otp_record = await repository_otp_auth.get_otp_record_by_id(
        current_user, db, otp_record_id
    )
    ...
    otp_password, remaining_time = await cor_otp.generate_and_verify_otp(
        otp_record.private_key, current_user
    )
...
async def create_otp_record(
    ...
    **Create a new otp record. / Создание записи** \n
    ...
    :param db: The database session. Dependency on get_db.
    :type db: AsyncSession, optional
    :return: The created OTPRecordResponse object representing the new otp record with current OTP.
    :rtype: OTPRecordResponse
    """
    otp_record = await repository_otp_auth.create_otp_record(body, db, current_user)
    otp_password, remaining_time = await cor_otp.generate_and_verify_otp(
        otp_record.private_key, current_user
    )
...
async def update_otp_record(
    ...
    **Update an existing otp record. / Обновление данных otp записи** \n
    ...
    :param db: The database session. Dependency on get_db.
    :type db: AsyncSession, optional
    :return: The updated ResponseRecord object representing the updated record with current OTP.
    :rtype: OTPRecordResponse
    ...
    otp_record = await repository_otp_auth.update_otp_record(
        otp_record_id, body, current_user, db
    )
    if otp_record is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Record not found"
...
async def remove_otp_record(
    ...
    **Remove a record. / Удаление записи** \n
    ...
    :raises HTTPException 404: If the record with the specified ID does not exist.
    """
    otp_record = await repository_otp_auth.delete_otp_record(
        current_user, db, otp_record_id
    )
    if otp_record is None:
        raise HTTPException(
